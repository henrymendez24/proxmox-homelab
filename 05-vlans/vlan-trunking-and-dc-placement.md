...
Outcome: DC (henry.local) now lives on the Servers VLAN (20) at 192.168.20.10, with working gateway, internet routing, and DNS. The full tagged-VLAN path from VM → VLAN-aware bridge → OPNsense trunk → firewall → WAN is verified.

1. Starting Point / Environment
Component	Details
...
2. The Core Concept (the lesson that anchors everything)
...
Assigning a VM 192.168.20.10 does not put it on VLAN 20. The VM's virtual NIC must be tagged for VLAN 20 on a VLAN-aware bridge, and that bridge must trunk the tag to OPNsense, where the VLAN 20 interface lives. If any link in that chain doesn't carry the tag, the traffic silently dies.

3. Problems Hit & How They Were Solved
...
Fix: Enabled VLAN aware on vmbr1 (adds bridge-vlan-aware yes, bridge-vids 2-4094) and applied live with ifreload -a.
Tagged the VM's NIC: VM 100 → Hardware → Network Device → VLAN Tag = 10.
Verified live bridge state:
bash
...
Interface naming reference (<fw><role> + <VMID>i<netN> / p<portN>):
...
Result: From the Proxmox host, ping 192.168.10.1 now succeeded (0% loss) — proving the VLAN path from host → OPNsense was good.
But from inside the DC it still failed. Next layer.
...
Fix — build a proper trunk on OPNsense's LAN NIC. OPNsense's net1 needs to be a trunk carrying VLAN 1 as native/untagged plus VLANs 10/20/30/40 tagged. Safety first (this reconfigures the live router's LAN port):
...
bash
  # (firewall already disabled on net1 via GUI, same fwbr reason as the DC)
...
A full Shutdown → Start of VM 101 applied it (brief router downtime; covered by the out-of-band paths). Verified:
  ...
  tap101i1    1   PVID Egress Untagged
              ...
              20
              30
              40                       ← native VLAN 1 + tagged 10/20/30/40 ✅
...
Fix: Added a pass rule on the interface:
  Firewall → Rules → Trusted → Add
    Action: Pass | Protocol: any | Source: Trusted net | Destination: any
  → Save → Apply
...
nslookup google.com    → resolved via localhost (DC's own DNS)       (DNS)
...
Inside Windows: static IP 192.168.20.10/24, gw 192.168.20.1, DNS 127.0.0.1 (.10 is below the Kea DHCP pool of .100–.200, so no conflict)
...
6. Final Architecture
   ...
   Internet ── WAN ──► vmbr0 ──► OPNsense (VM 101)                                   │
                        │          │  net1 = TRUNK: native VLAN 1 + tagged 10/20/30/40│
                        │          ▼                                                 │
                        │        vmbr1  (VLAN-aware bridge, bridge-vids 2-4094)       │
                        │          │                                                 │
                        │          ▼                                                 │
                        │     tap100i0 (tag=20)  ──►  DC / VM 100 (192.168.20.10)     │
                        │        [firewall wrapper disabled → clean tag]             │
                        └────────────────────────────────────────────────────────────┘

  VLAN interfaces (in OPNsense, parent vtnet1 = net1):
    Trusted 10 → 192.168.10.1     Servers 20 → 192.168.20.1
    IoT     30 → 192.168.30.1     Lab     40 → 192.168.40.1
7. Key Takeaways
...
Trunk config lives on the OPNsense VM's NIC via qm set <vmid> -netN '...,trunks=10;20;30;40' (quote the semicolons).
...
8. Still To Do (next sessions)
 ...
 Retire the VirtualBox DC01 (adlab.local) to consolidate to the single henry.local domain.
